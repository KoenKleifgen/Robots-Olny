...
import time           # For adding delays and timing features
from typing import List, Dict, Optional  # For type hints that help document what kind of data we expect
from dataclasses import dataclass        # For creating classes that primarily hold data
import json           # For handling JSON data (though not currently used in the code)
from enum import Enum # For creating sets of named constants
import os            # For operating system operations like clearing the screen

# Define color codes to make text colorful in the terminal
class Colors:
    ...
    RED = '\033[91m'     # Red
    ENDC = '\033[0m'     # Reset color back to default
    BOLD = '\033[1m'     # Make text bold

# Function to clear the terminal screen
def clear_screen():
    # Use 'cls' command for Windows, 'clear' for Unix-based systems
    os.system('cls' if os.name == 'nt' else 'clear')

# Function to print text character by character for dramatic effect
def dramatic_print(text: str, delay: float = 0.03):
    """
    Print text one character at a time with a small delay for dramatic effect
    text: The text to print
    delay: How long to wait between each character (in seconds)
    """
    for char in text:
        print(char, end='', flush=True)  # Print without newline and force output
        time.sleep(delay)                # Wait briefly
    print()  # Print newline at the end

# Define the available character classes as constants
class CharacterClass(Enum):
    WARRIOR = "Warrior"   # Strong melee fighter
    MAGE = "Mage"        # Powerful spell caster
    ROGUE = "Rogue"      # Stealthy and agile
    PALADIN = "Paladin"  # Holy warrior with healing

# Define the types of items that can exist in the game
class ItemType(Enum):
    WEAPON = "weapon"     # Weapons for combat
    POTION = "potion"    # Consumable items for healing
    ARTIFACT = "artifact" # Special items with unique effects
    SCROLL = "scroll"    # Magic scrolls for special abilities

# Class to define special abilities that characters can use
@dataclass
class Ability:
    name: str                    # Name of the ability
    description: str             # What the ability does
    damage_multiplier: float     # How much extra damage it does
    cooldown: int               # How many turns until it can be used again
    current_cooldown: int = 0    # Current turns remaining until ability is ready

# Class to hold character statistics
@dataclass
class Stats:
    strength: int               # Affects physical damage
    agility: int               # Affects dodge and critical hits
    magic: int                 # Affects spell damage
    health: int                # Current health points
    max_health: int            # Maximum possible health
    mana: int = 100           # Current magic points
    max_mana: int = 100       # Maximum possible magic points
    critical_chance: float = 0.1  # Chance to deal double damage

# Class to define items that can be found or used
@dataclass
class Item:
    name: str                  # Name of the item
    item_type: ItemType       # What kind of item it is
    description: str          # Description of what it does
    power: int               # How powerful the item is
    uses: Optional[int] = None  # How many times it can be used (if applicable)
    special_effect: Optional[str] = None  # Any special effects

# Class to define enemies that players can fight
@dataclass
class Enemy:
    name: str                 # Enemy name
    health: int              # Current health points
    max_health: int         # Maximum possible health
    damage: int             # How much damage they deal
    description: str        # Description of the enemy
    special_ability: Optional[str] = None  # Special attack name
    special_ability_chance: float = 0.2    # Chance to use special attack
    loot: List[Item] = None  # Items that can be dropped when defeated

    # Initialize empty loot list if none provided
...
class Player:
    def __init__(self, name: str, character_class: CharacterClass):
        ...
        character_class: Type of character (Warrior, Mage, etc.)
        """
        self.name = name
        self.character_class = character_class
        self.inventory: List[Item] = []  # List to hold items
        self.stats = self._initialize_stats()  # Set up starting statistics
        self.story_choices: Dict[str, str] = {}  # Track choices made during the story
        self.abilities = self._initialize_abilities()  # Set up character abilities
        self.level = 1       # Starting level
        self.experience = 0  # Starting experience points
        self.gold = 50      # Starting gold amount
